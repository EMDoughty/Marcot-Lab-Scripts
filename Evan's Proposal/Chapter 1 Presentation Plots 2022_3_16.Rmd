...
## Hypercarnivore + Mesocarnivore

```{r, echo = FALSE}
#quartz(width=6.89)
#par(mfrow=c(2,1), mar=c(4,4,0.5,0.5)) #, mgp=c(2, 1,0))
shoulderPlot(measure.mat = pred.data.hyper.meso, plot.y = "bodyMass", intervals = intervals, occs = occs, this.rank = "species", 
             bigList = bigListPred, shortFam = shortFamPred, repIntTaxa = repIntTaxa, quants = bm_quants_pred_hyper.meso,
             optList_bm_median = optList_bm_median_pred_hyper.meso, xlim = c(65, 0), ylim = c(-1,  2.75),
             specOcc.col = "gray0", specOcc.alpha = 0.5,
             plot.breaks = TRUE,  break.text = FALSE, break.col = "firebrick4", #manual.breaks = c(57, 41, 25),
             ylab = "log Bodymass (kg)", xlab = "Time (Ma)", xaxp = c(75,0,15), cex.axis = 1, cex.lab = 1, 
             do.subepochs = TRUE,  do.quants = TRUE,
...
## Hypercarnivore + Hypocarnivore

```{r, echo = FALSE}
#quartz(width=6.89)
#par(mfrow=c(2,1), mar=c(4,4,0.5,0.5)) #, mgp=c(2, 1,0))
shoulderPlot(measure.mat = pred.data.hyper.hypo, plot.y = "bodyMass", intervals = intervals, occs = occs, this.rank = "species", 
...
#quartz(width=6.89)
#par(mfrow=c(2,1), mar=c(4,4,0.5,0.5)) #, mgp=c(2, 1,0))
shoulderPlot(measure.mat = pred.data.meso.hypo, plot.y = "bodyMass", intervals = intervals, occs = occs, this.rank = "species", 
             bigList = bigListPred, shortFam = shortFamPred, repIntTaxa = repIntTaxa, quants = bm_quants_pred_meso.hypo,
             optList_bm_median = optList_bm_median_pred_meso.hypo, xlim = c(65, 0), ylim = c(-1,  2.75),
             specOcc.col = "gray0", specOcc.alpha = 0.5,
             plot.breaks = TRUE,  break.text = FALSE, break.col = "firebrick4", #manual.breaks = c(57, 41, 25),
             ylab = "log Bodymass (kg)", xlab = "Time (Ma)", xaxp = c(75,0,15), cex.axis = 1, cex.lab = 1, 
             do.subepochs = TRUE,  do.quants = TRUE,
...
```{r, echo = FALSE}
countBox_pred_meso.hypo <- apply(countCube_pred_hyper.hypo, c(1,2), quantile, probs=c(0.025, 0.5, 0.975), na.rm=TRUE) 
...
```{r}
#######
#Code to checkdegree of sub sampling

subset.count <- vector()
for(xx in seq(1, length(repIntOccs),1)) subset.count[xx] <- length(repIntOccs[[xx]]$`1 Ma`)
boxplot(subset.count, ylim = c(100, 500))

#collect data for # occurences in bin
nonsubset.count <- matrix(nrow = length(repIntOccs), ncol = length(repIntOccs[[1]]))
for(xx in seq(1, length(repIntOccs),1)) 
{
  for(yy in seq(1, length(repIntOccs[[xx]]),1)) 
  {
    nonsubset.count[xx, yy] <- length(repIntOccs[[xx]][[yy]])
  }
}

colnames(nonsubset.count) <- names(repIntOccs[[1]])
