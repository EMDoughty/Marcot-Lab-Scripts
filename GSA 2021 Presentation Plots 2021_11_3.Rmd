...
herbivore.size.cat <- c(-Inf, 0.69897, 1.39794, 2.176091, 2.69897, 3.0, Inf) #Janis 2000  max(measure.mat$bodyMass, na.rm=TRUE)
...
colnames(pred.data) <- c("family", "taxon",	"max_ma","min_ma",	"m1L",	"rbl",	"bodyMass",	"BM_extant_reg")
...
yval <- t(as.matrix(pred.preyDivCompare[2:6,]))
#yval <- cbind(yval, 0)
...
```{r}
pred.prey.DivFirstDiff <- getDiversity1stDiff(data.mat = pred.preyDivCompare[2:6,], intervals = intervals)
...
```{r}
measure.mat <- measure.mat.species
```

```{r}
pred.data <- pred.data.species
```
...
```{r}
bm_quants_ung <- apply(sapply(repIntTaxa_ung, function(y) sapply(y, function(x) quantile(measure.mat[as.character(x),"bodyMass"], probs=c(0, 0.25, 0.5, 0.75, 1.0), na.rm=TRUE)), simplify = "array"), c(1,2), median, na.rm=TRUE)

bm_quants_pred <- apply(sapply(repIntTaxa_pred, function(y) sapply(y, function(x) quantile(pred.data[as.character(x),"bodyMass"], probs=c(0, 0.25, 0.5, 0.75, 1.0), na.rm=TRUE)), simplify = "array"), c(1,2), median, na.rm=TRUE)
```
...
shoulderPlot(measure.mat = measure.mat, plot.y = "bodyMass", intervals = intervals, occs = occs, this.rank = "species", 
             ...
             plot.breaks = TRUE, manual.breaks = c(61, 57, 51, 39, 33, 25, 5), break.text = FALSE, break.col = "blue4",
...
shoulderPlot(measure.mat = pred.data, plot.y = "bodyMass", intervals = intervals, occs = occs, this.rank = "species", 
             ...
             plot.breaks = TRUE, manual.breaks = c(57, 41, 25), break.text = FALSE, break.col = "firebrick4",
...
plotStackedRichness(this.box=prop, intervals=intervals, reorder.taxa = TRUE, do.log=FALSE, overlay.labels=FALSE, numbers.only=FALSE, legend=FALSE, xlim = c(65, min(intervals, na.rm=TRUE)))
```
...
plotStackedRichness(this.box=prop, intervals=intervals, reorder.taxa = TRUE, do.log=FALSE, overlay.labels=FALSE, numbers.only=TRUE, legend=FALSE, xlim = c(65, min(intervals, na.rm=TRUE)))
```
...
repIntOccs <- repIntOccs_ung

splat <- function (this.rep) {
  occ.box <- cbind(n_occs_ung=sapply(this.rep, function(this.intv) sum(occs[occs$occurrence_no %in% this.intv,]$family %in% focal.family)), 
                   ...
                   n_cols_ung=sapply(this.rep, function(this.intv) length(sort(unique(occs$collection_no[occs$occurrence_no %in% this.intv & occs$family %in% focal.family])))),
  ...
  occ.box <- cbind(occ.box, prop_occ_ung=round(occ.box[,"n_occs_ung"]/occ.box[,"n_occs"], digits=3), prop_occ_carn=round(occ.box[,"n_occs_carn"]/occ.box[,"n_occs"], digits=3), prop_occ_rod=round(occ.box[,"n_occs_rod"]/occ.box[,"n_occs"], digits=3))
  occ.box <- cbind(occ.box, prop_col_ung=round(occ.box[,"n_cols_ung"]/occ.box[,"n_cols"], digits=3), prop_col_carn=round(occ.box[,"n_cols_carn"]/occ.box[,"n_cols"], digits=3), prop_col_rod=round(occ.box[,"n_cols_rod"]/occ.box[,"n_cols"], digits=3))
}
occ.box <- apply(sapply(repIntOccs, splat, simplify="array"), c(1,2), quantile, probs=c(0.025, 0.50, 0.975))
```

```{r}
plot(apply(intervals, 1, mean), occ.box[2,,"prop_occ_ung"], xlim=c(65, 0), ylim=c(0, 0.75), type="n", xlab="Time (Ma", ylab="Proportion of occurrences with taxon")
...
rep.abundance<- getRepIntData(#bigList, 
  #col.nam = NULL,
  occs=occs,
  intervals = NULL,
  int_length = 2,
...
rep.abundance <- rep.abundance$RepOccs
...
this.repIntOccs <- rep.abundance[[1]]

occs.count <- lapply(this.repIntOccs, function(x) table(occs$accepted_name[occs$occurrence_no %in% x] %in% bigList$accepted_name))

test.mat <- matrix(unlist(occs.count), ncol = 2, nrow = nrow(intervals))
colnames(test.mat) <- c(FALSE, TRUE)
rownames(test.mat) <- rownames(intervals)

test.mat <- test.mat[nrow(test.mat):1, ]
...
```{r}
occs.count <- lapply(this.repIntOccs, function(x) table(occs$accepted_name[occs$occurrence_no %in% x] %in% bigListPred$accepted_name))
