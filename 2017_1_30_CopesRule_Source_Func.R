...
head(occs)
	...
	mat_ranges <- matrix()
	mat_ranges <- ranges
	mat_ranges <- intervals
	
	#code from Bapst Site will likely not work as fuctions--freRate and getSampDisc--are for discrete data
	freqRat(timeData = ranges,plot=TRUE) #Error in freqRat(timeData = ranges, plot = TRUE) : timeList[[2]] not in intervals numbered from first to last (1 to infinity)
	freqRat()
