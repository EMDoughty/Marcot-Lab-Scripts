...
{
  #How to separate unique occurances of species in countBox[2,,]
  if(class(countBox) == "list"){
    countBox[2,,] #median for each BM catagory per time interval
    dimnames(countBox)[[3]]  
    ...
    dimnames(countBox)[[3]] <- str_remove(dimnames(countBox)[[3]], " Ma")
  
    regimeBM <- countBox[2,,which(as.double(dimnames(countBox)[[3]]) > breaks[1])]
    regimeBM.list[[1]] <- apply(regimeBM, c(1), median)
    rm(regimeBM)
    ...
    for(mm in seq(2,length(breaks),1))
    {
      ...
      regimeBM <- countBox[2,,which(as.double(dimnames(countBox)[[3]]) < breaks[mm-1] 
                                    & as.double(dimnames(countBox)[[3]]) > breaks[mm])]
      regimeBM.list[[mm]] <- apply(regimeBM, c(1), median)
  
      rm(regimeBM)
    }  
  
    regimeBM <- countBox[2,,which(as.double(dimnames(countBox)[[3]]) < breaks[length(breaks)])]
    ...
    {
      hist.list[[jj]] <- hist(regimeBM.list[[jj]], main= NULL, breaks = bmBreaks, 
                              las=1, col = breakCol, freq = regimeFreq, xlab="log Body Mass (kg)")
    }
    ...
    if(netPlotType == "pos/neg")
    {
      ...
      quartz()
      par(mfrow=c(2, length(hist.list)/2+1))
